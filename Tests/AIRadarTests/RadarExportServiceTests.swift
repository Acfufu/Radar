...
struct RadarExportServiceTests {
    ...
    func destinationAndCancellationCleanup() async throws {
        ...
        #expect(try FileManager.default.contentsOfDirectory(atPath: root.path).allSatisfy { !$0.hasPrefix(".AIRadarExport-") })
    ...
    func destinationSafetyAndReplacement() async throws {
        ...
        #expect(ExportDataset.allCases.map(\.rawValue) == ["models", "benchmark-runs", "community-ratings", "source-status", "rendered-warnings", "codex-station-status", "intelligence-efficiency", "fast-radar-history", "raw-samples"])
        #expect(ExportDataset.normalized.map(\.rawValue) == ["models", "benchmark-runs", "community-ratings", "source-status", "rendered-warnings", "codex-station-status", "intelligence-efficiency", "fast-radar-history"])
    }
