...
struct RadarAppRuntimeTests {
    ...
    func disabledManualRefresh() async throws {
        let root = FileManager.default.temporaryDirectory.appending(path: "AIRadar-DisabledRefresh-\(UUID().uuidString)", directoryHint: .isDirectory)
    ...
    func failedHistoryDeletionResumesSynchronization() async throws {
        ...
        await runtime.start()
        for _ in 0..<200 where warningReader.readCount < 1 || efficiencyReader.readCount < 1 || fastRadarReader.readCount < 1 {
            await Task.yield()
        }
        ...
        await runtime.refresh()
        for _ in 0..<200 where warningReader.readCount < 2 { await Task.yield() }
