...
Build with `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer ./Scripts/build-app.sh debug`; bind the executable with `shasum -a 256 .build/app/AIRadar.app/Contents/MacOS/AIRadar`; then launch with `/usr/bin/open -n -F --env HOME=<isolated>/home --env CFFIXED_USER_HOME=<isolated>/home --env RADAR_DATA_ROOT=<isolated>/data --env RADAR_FIXTURE_MODE=ui --env RADAR_UI_STATE=<state> [--env RADAR_UI_SOURCE=<source>] [--env RADAR_UI_DESTINATION=<destination>] .build/app/AIRadar.app`. Todo 8 process smoke covered Information Overview, Claude Export, Codex Intelligence Center, and SWE Models with sustained PIDs and targeted cleanup.
