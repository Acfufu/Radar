...
应用展示和整理第三方 Radar 数据，不自行运行 Benchmark。
...
项目所有者已于 2026-07-15 明确批准自动访问公开来源、合理缓存、本地历史留存和应用内数据再展示。Release 构建默认启用在线来源，继续遵守来源的缓存 Header、速率限制和访问控制；不绕过认证、挑战或其他访问限制。
...
- 无后台 Agent；
- 无未使用的平台化抽象；
- 在线来源授权边界已确认；公开 Release 构建默认启用自动同步、合理缓存、本地历史留存和应用内再展示。
