                ...
                GroupBox(WorkspaceCopy.quotaTitle(for: projection.source)) {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        ForEach(quotas) { quota in
                            LabeledContent(quota.windowLabel) { VStack(alignment: .trailing) { Text(quotaValue(quota)); Text(quota.resetDescription ?? "未提供估算依据").font(.caption).foregroundStyle(.secondary) } }
                        }
