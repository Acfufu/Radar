...
/// Sidecar reader for the upstream radar-insights dataset (spec §5.6).
/// Follows the rendered-reader sidecar pattern: the main benchmark chain
/// never touches this endpoint; the coordinator drives `read()` through
/// `CodexRenderedSyncCore`.
protocol RadarInsightsReading: Sendable {
    func read() async throws -> RadarInsightsDataset
    func cancel() async
}

actor RadarInsightsAdapter: RadarInsightsReading {
    /// Single fixed endpoint (spec §5.0 whitelist: codexradar.com same-origin
    ...
    static let endpoint = URL(string: "https://codexradar.com/api/radar-insights")!
    ...
    /// measured ~29KiB on 2026-09-08; §5.0 sets 8MiB for every sidecar).
    ...
    init(
        transport: any HTTPTransport = URLSessionHTTPTransport(maxBodyBytes: RadarInsightsAdapter.maximumResponseBytes),
    ...
    static func production(dataRoot: URL) -> RadarInsightsAdapter {
        RadarInsightsAdapter(
    ...
    func read() async throws -> RadarInsightsDataset {
        ...
        do {
            let dataset = try RadarInsightsParser.parse(
                payload.data,
                sourceID: sourceID,
                fetchedAt: fetchedAt
            )
            ...
            return dataset
        } catch let error as RadarInsightsParseError {
            switch error {
            case .malformedJSON:
                await saveRaw(payload.data, outcome: .decodingFailed, at: fetchedAt)
            case .unexpectedBenchmark:
